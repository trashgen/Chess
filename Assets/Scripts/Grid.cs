...
public class Grid<TGridObject> where TGridObject : class {
    ...
    public Vector3 CalcVector3(Vector3 pos) {
        return pos * cellSize + Vector3.one * (cellSize * .5f);
    }
    
    public Vector3 CalcVector3(int x, int y) {
        return new Vector3(x, y) * cellSize + Vector3.one * (cellSize * .5f);
    }
